...
**Última actualización:** 2026-09-19 (BUG-007 mitigado: Cat L.Mat delega a `LiberarBomCrudoResolver`; BUG-012 sigue abierto).
...
| BUG-007 | P1 | L.Mat / AX | Dos SQLs incompatibles L.Mat CRUDO → AX | Mitigado: `CatCodificacionController::queryLmatDesdeTi` delega a `LiberarBomCrudoResolver::query` (EXISTS, sin `limit(50)`). Salón de Cat (`Departamento` / `TelarId`) o `todosLosAliasesAx()`. Shape UI intacto: `{bomId, bomName}` → `listaLmat`. Tests: `CatCodificacionLmatQueryTest`, `LiberarBomCrudoResolverTest`, `LiberarBomYNombreTest`. | Mismo `(item, size, salon)` lista las mismas L.Mat en Liberar y Peso muestra | mitigado | 2026-09-19. Dual `ReqModelosCodificados`/`CatCodificados` es **BUG-012** (follow-up; no este PR). No se creó `LmatCrudoQueryService`: el dueño es el resolver. |
