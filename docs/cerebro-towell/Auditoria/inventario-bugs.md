...
=======
**Última actualización:** 2026-09-19 (BUG-006/008 mitigados; BUG-009 parcial via LiberarMarbetesCalculator; BUG-004 wontfix/`CreaProd` intencional).
>>>>>>> be4f4de6 (refactor(planeacion): extraer LiberarMarbetesCalculator de LiberarOrdenesController)
...
=======
| BUG-004 | P1 | Planeación / AX | Liberar fuerza `CreaProd = 1` (re-encola AX) | `LiberarOrdenesController` `CreaProd = 1` en persistencia y payload Cat; contraste `OrdenDeCambioFelpaController` omite `CreaProd` en update | Re-liberar vuelve a encolar producción en AX | confirmado (wontfix / intencional) | Franco (GO): `CreaProd = 1` al liberar es **correcto**. No “arreglar”. Extraer marbetes no toca este campo. |
