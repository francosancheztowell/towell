...
**Última actualización:** 2026-09-19 (BUG-009 parcial: LiberarCamposEditablesService; `CreaProd=1` al liberar no se tocó).
...
| BUG-004 | P1 | Planeación / AX | Liberar fuerza `CreaProd = 1` (re-encola AX) | `LiberarOrdenesController` asigna `$registro->CreaProd = 1`; payload de `LiberarCatCodificadosWriter::actualizar` incluye `'CreaProd' => 1`. Contraste: `OrdenDeCambioFelpaController` omite `CreaProd` en update. Tests: `LiberarCatCodificadosWriterTest::test_actualizar_fuerza_crea_prod_a_uno_aunque_la_fila_ya_exista`, `LiberarOrdenesLiberarTest` | Re-liberar vuelve a encolar producción en AX | wontfix | 2026-09-19: Franco/towellin — `CreaProd = 1` al liberar es **intencional**. No “arreglar” omitiendo el flag en filas existentes. |
| BUG-005 | P1 | Engomado | `actualizarStatus` → En Proceso sin exigir Urdido Finalizado | Mitigado: `ProgramarEngomadoController::actualizarStatus` delega a `ProgramBoardActionService::changeStatus`. Guardia en `productionBlockReasonForOrder` (Urdido `Status === Finalizado`). Feature: `tests/Feature/ProgramBoardStatusGuardsTest.php` | POST ya no salta la regla | mitigado | 2026-09-19. GET `verificar-en-proceso` eliminado (BUG-017). |
...
| BUG-009 | P1 | Arquitectura | Fat controller LiberarOrdenes ~2941 LOC | `LiberarOrdenesController` (~1045 LOC) + `LiberarMarbetesCalculator` + `LiberarBomCrudoResolver` + `LiberarCatCodificadosWriter` + `LiberarFlogSugeridoService` + `LiberarCodigoDibujoResolver` + `LiberarProgramaScheduling` + `LiberarValidacionesService` + `LiberarHilosCatalogo` + `LiberarCamposEditablesService` | Imposible testear/revisar; mezcla AX, Cat, BOM, folio | confirmado (parcial) | 2026-09-19: extraído `LiberarCamposEditablesService` (`guardarCamposEditables`). Tests: `LiberarCamposEditablesTest` + `LiberarOrdenesLiberarTest`. Grasa restante: `liberar()` (~508 LOC) + wrappers HTTP delgados. `CreaProd=1` al liberar no se tocó (BUG-004 wontfix). |
...
| BUG-012 | P1 | Codificación | Dual `ReqModelosCodificados` vs `CatCodificados` | `CodificacionController` + `catalagos/catalogoCodificacion.blade.php` vs `CatCodificacionController` + `catcodificacion/` | Dos superficies HTTP, Excel duplicado, rutas parecidas | **no es duplicado** (owner 2026-09-25): catálogos = `ReqModelosCodificados` (modelos), codificación = `CatCodificados` (órdenes); se quedan las dos | Sin cambio de superficie; 19-06 migra ambas pantallas por separado. El 404 de `/planeacion/catalogos/catalogoCodificacion` se cubrió con redirect (`42837fa1`). |
...
| BUG-016 | P2 | Urd/Eng | Engomado legacy sin límite 2× En Proceso | Mitigado: `ProgramBoardActionService::productionBlockReasonForOrder` aplica tope 2× En Proceso por lane también a Engomado (`MaquinaEng` / West Point). POST `actualizar-status` delega al service. Feature: `ProgramBoardStatusGuardsTest::test_engomado_respects_max_two_en_proceso_per_machine` | Misma regla Urdido/Engomado/Livewire | mitigado | 2026-09-19. GET `verificar-en-proceso` eliminado (BUG-017). |
| BUG-017 | P2 | Engomado | `verificarOrdenEnProceso` falla abierto + no se usa | Mitigado: rutas GET `…/verificar-en-proceso` y métodos `verificarOrdenEnProceso` eliminados (Urdido y Engomado). Ripgrep: Livewire no las llamaba; Blade Urdido solo vía `/legacy` (ahora 301). Tests: `ProgramBoardStatusGuardsTest::test_verificar_en_proceso_endpoints_are_gone`, `ProgramBoardRouteContractTest::test_verificar_en_proceso_routes_are_removed` | Ya no hay GET fail-open que contradiga el service | mitigado | 2026-09-19. Mutaciones siguen solo en `ProgramBoardActionService::productionBlockReason`. |
...
| BUG-022 | P2 | Modelos | `UrdEngNucleos` duplicado (case paths) | `app/Models/UrdEngomado/UrdEngNucleos.php` vs `app/Models/urdengomado/UrdEngNucleos.php` | En Linux/CI el autoload puede divergir de Windows | **resuelto** (duplicado borrado en `160613b`; guardia `tests/Unit/Helpers/RutasSinColisionDeMayusculasTest.php`, fase 20-01) | Un path canónico. |
...
1. **Liberar LOC real = 2941** (no ~2537) — BUG-009. 2026-09-19: ~1045 tras extraer `LiberarCamposEditablesService`. Grasa restante: `liberar()` (~508 LOC) + wrappers HTTP delgados.
...
| BUG-031 | P1 | Engomado | Ruta .legacy sirve Livewire; default index = Blade debil | Mitigado: `index()` → Livewire. `GET …/legacy` redirige 301 al default (Urdido y Engomado; Urdido `/livewire` ya era 301). Blade clásico queda archivo, no se sirve. Tests: `ProgramBoardStatusGuardsTest::test_legacy_boards_redirect_to_the_livewire_default` | Default planta = Livewire; `/legacy` no es tablero más débil | mitigado | 2026-09-19. El botón Livewire “Anterior” apunta a `.legacy` y recae en el mismo board. |
...
Resumen actualizado: P0=3, P1=13 (+BUG-031; **005/011/031 mitigados**), P2=13 (+029/030/032/033; **016/017 mitigados**), P3=5 (+034). Total **34**.

---

## Slice tablero Urd/Eng (BUG-005 / 011 / 016 / 017 / 031) — 2026-09-19
...
- `GET .../legacy` redirige 301 al default Livewire (elección: no servir Blade interactivo más débil). `GET /urdido/programar-urdido/livewire` ya redirigía 301. El enlace navbar “Anterior” recae en el mismo board.
...
- GET `verificar-en-proceso` **eliminado** (fail-open / “restricción eliminada”; no lo llamaba Livewire; Blade Urdido solo vía `/legacy`). No se reabrió una segunda regla de mutación.
