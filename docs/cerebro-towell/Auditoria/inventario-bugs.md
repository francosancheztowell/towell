...
**Última actualización:** 2026-09-19 (BUG-009 parcial + LiberarBomCrudoResolver; BUG-007 no unificado con CatCodificacion).
...
| BUG-009 | P1 | Arquitectura | Fat controller LiberarOrdenes ~2941 LOC | `LiberarOrdenesController` (~2214 LOC) + `LiberarMarbetesCalculator` + `LiberarBomCrudoResolver` | Imposible testear/revisar; mezcla AX, Cat, BOM, marbetes | confirmado (parcial) | 2026-09-19: marbetes (#41) + cluster BomCrudo/L.Mat extraído. Tests: `LiberarBomCrudoResolverTest`, `LiberarBomYNombreTest`. Quedan Cat/flogs/folio en el controller. `CreaProd=1` al liberar no se tocó (BUG-004). |
...
1. **Liberar LOC real = 2941** (no ~2537) — BUG-009. 2026-09-19: ~2561 tras `LiberarMarbetesCalculator`; ~2214 tras `LiberarBomCrudoResolver` (mitigación parcial; Cat/flogs/folio siguen en el controller).
