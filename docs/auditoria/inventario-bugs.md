...
**Última actualización:** 2026-09-19 (BUG-009 parcial: LiberarCamposEditablesService).
...
| BUG-009 | P1 | Arquitectura | Fat controller LiberarOrdenes ~2941 LOC | `LiberarOrdenesController` (~1045 LOC) + `LiberarMarbetesCalculator` + `LiberarBomCrudoResolver` + `LiberarCatCodificadosWriter` + `LiberarFlogSugeridoService` + `LiberarCodigoDibujoResolver` + `LiberarProgramaScheduling` + `LiberarValidacionesService` + `LiberarHilosCatalogo` + `LiberarCamposEditablesService` | Imposible testear/revisar; mezcla AX, Cat, BOM, folio | confirmado (parcial) | 2026-09-19: extraído `LiberarCamposEditablesService` (guardar-campos). Grasa restante: `liberar()` (~508 LOC) + wrappers HTTP delgados. `CreaProd=1` intencional (BUG-004 wontfix). |
