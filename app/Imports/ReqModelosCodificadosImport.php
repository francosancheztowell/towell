...
class ReqModelosCodificadosImport implements ShouldQueue, SkipsEmptyRows, ToCollection, WithBatchInserts, WithChunkReading, WithEvents, WithStartRow
{
    ...
    private function D(array $assoc, array $vals, array $cands, ?int $posIdx1Based): ?Carbon
    {
        ...
        try {
            if (is_numeric($v)) {
                return Carbon::instance(ExcelDate::excelToDateTimeObject($v));
            }
            $v = trim((string) $v);
            if ($v === '') {
                return null;
    ...
    private function pick(array $assoc, array $vals, array $cands, ?int $posIdx1Based)
    {
        ...
        foreach ($cands as $k) {
            if (array_key_exists($k, $assoc)) {
                return $this->cleanExcelFormula($assoc[$k]);
            }
        }
        ...
        foreach ($cands as $k) {
            $nk = $this->normKey($k);
            if (array_key_exists($nk, $assocNorm)) {
                return $this->cleanExcelFormula($assocNorm[$nk]);
            }
        }
        ...
        foreach ($cands as $k) {
            ...
            foreach ($assoc as $kk => $vv) {
                if (str_contains($this->normKey($kk), $nk)) {
                    return $this->cleanExcelFormula($vv);
                }
            }
        }
        ...
        if ($posIdx1Based !== null) {
            $idx = $posIdx1Based - 1;
            if ($idx >= 0 && $idx < count($vals)) {
                return $this->cleanExcelFormula($vals[$idx] ?? null);
            }
        }
