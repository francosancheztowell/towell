...
class ReqProgramaTejidoSimpleImport implements ToModel, WithBatchInserts, WithChunkReading, WithHeadingRow
{
    ...
    {
        return 15;
    } // SQL Server limita a 2100 parámetros. Con ~100 campos, máximo 15 registros por batch

    public function chunkSize(): int
    {
        return 500;
    } // Leer Excel en chunks de 500 filas
    ...
    private function parseDate($value): ?string
    {
        ...
        try {
            ...
            // 5) DD-MMM (sin año)
            if ($out = $this->parseDiaMesNombre($s)) {
                return $out;
            }

            // 6) DD-MMM-YYYY (con año)
            if ($out = $this->parseDiaMesNombreConAno($s)) {
                return $out;
            }
    ...
    {
        if (! empty(self::$schemaStringLimits)) {
            return;
        }
