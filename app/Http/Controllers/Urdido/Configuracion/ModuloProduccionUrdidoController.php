...
class ModuloProduccionUrdidoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Si hay un julio con Julios = 6, se generan 6 registros
        // Si hay 2 julios con Julios = 2 cada uno, se generan 4 registros total (2+2)
        $totalRegistros = 0;
        if ($julios->count() > 0) {
            ...
            try {
                ...
                if (count($registrosACrear) > 0) {
                    ...
                    Log::info('No se crearán registros adicionales - ya existen todos los necesarios', [
                        'folio' => $orden->Folio,
                        'total_registros_necesarios' => $totalRegistros,
                        'registros_por_hilos' => $registrosPorHilos,
                    ]);
                }
            } catch (\Throwable $e) {
