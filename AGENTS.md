...
- En **reporte fallos paros** (`mantenimiento/reporte-fallos-paros/index`, mismo contenido en `/mantenimiento/solicitudes`): `GET api/mantenimiento/paros/index` solo devuelve paros cuyo **`Depto` coincide con el área del usuario** (`Auth::user()->area` o `SYSUsuario.area`); sin área configurada no se devuelven filas. El filtro **Solo mis solicitudes** va **desmarcado** por defecto (`mostrarSoloMisSolicitudes` inicia en `false`); **Limpiar** debe dejar ese checkbox desmarcado. El combo **Área** (`Depto`) debe **preseleccionarse** con `Auth::user()->area` cuando ese valor exista entre los departamentos de los datos; **Limpiar** restaura Status/Máquina y deja **Área** en la del usuario (si sigue siendo opción válida), no forzar otra vez «solo mis solicitudes».
