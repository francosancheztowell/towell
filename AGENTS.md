...
- En **reporte fallos paros** (`mantenimiento/reporte-fallos-paros/index`): el filtro **Solo mis solicitudes** va **desmarcado** por defecto (`mostrarSoloMisSolicitudes` inicia en `false`); **Limpiar** debe dejar ese checkbox desmarcado. El combo **Área** (`Depto`) debe **preseleccionarse** con `Auth::user()->area` cuando ese valor exista entre los departamentos de los datos; **Limpiar** restaura Status/Máquina y deja **Área** en la del usuario (si sigue siendo opción válida), no forzar otra vez «solo mis solicitudes».
