...
- En clases bajo `App\...`, usar `use Carbon\Carbon` (o `CarbonInterface`); en métodos PHP muy largos con muchas ramas, extraer lógica a métodos privados y retorno explícito suele aliviar analizadores/IDE (p. ej. avisos "too many types").
...
- En **Liberar órdenes**, `obtenerCodigoDibujo` lee `CatCodificados` (no `ReqModelosCodificados`): el front puede enviar `combinations` como `itemId::inventSizeId::salon` (fila con `data-salon-tejido-id` / `SalonTejidoId` alineado con `Departamento`); el backend resuelve en orden: ItemId+InventSizeId+Departamento → ItemId+Departamento (útil si el tamaño AX no coincide) → ItemId+InventSizeId; en cada intento, `Id` descendente y primer `CodigoDibujo` no vacío.
