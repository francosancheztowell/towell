...
class ModuloProduccionUrdidoControllerTest extends TestCase
{
    ...
    /**
     * Con 10 renglones el operador no ve los de arriba y arranca en el 7,
     * dejando huecos. El servidor lo rechaza y le dice cual sigue.
     */
    public function test_no_deja_capturar_saltandose_renglones(): void
    {
        $this->escenarioDiezRenglones();
        $controller = $this->controladorUrdido();

        // renglones 1 y 2 capturados; se intenta arrancar el 7
        $response = $controller->actualizarHoras(Request::create('/h', 'POST', [
            'registro_id' => 107, 'campo' => 'HoraInicial', 'valor' => '08:00',
        ]));
        $payload = $response->getData(true);

        $this->assertFalse($payload['success']);
        $this->assertSame(422, $response->getStatusCode());
        $this->assertTrue($payload['fuera_de_orden']);
        $this->assertSame(3, $payload['renglon_siguiente'], 'Debe mandarlo al renglon 3.');
        $this->assertNull(DB::connection('sqlsrv')->table('UrdProduccionUrdido')->where('Id', 107)->value('HoraInicial'));
    }

    /** El siguiente renglon libre si se puede capturar. */
    public function test_permite_capturar_el_siguiente_renglon_libre(): void
    {
        $this->escenarioDiezRenglones();
        $controller = $this->controladorUrdido();

        $response = $controller->actualizarHoras(Request::create('/h', 'POST', [
            'registro_id' => 103, 'campo' => 'HoraInicial', 'valor' => '08:00',
        ]));

        $this->assertTrue($response->getData(true)['success']);
        $this->assertSame('08:00', DB::connection('sqlsrv')->table('UrdProduccionUrdido')->where('Id', 103)->value('HoraInicial'));
    }

    /** Una fila ya iniciada sigue siendo editable aunque haya huecos antes. */
    public function test_permite_corregir_una_fila_ya_iniciada(): void
    {
        $this->escenarioDiezRenglones();
        $controller = $this->controladorUrdido();

        $response = $controller->actualizarHoras(Request::create('/h', 'POST', [
            'registro_id' => 102, 'campo' => 'HoraFinal', 'valor' => '09:45',
        ]));

        $this->assertTrue($response->getData(true)['success']);
    }

    private function escenarioDiezRenglones(): void
    {
        DB::connection('sqlsrv')->table('UrdProgramaUrdido')->insert([
            'Id' => 9, 'Folio' => '00700', 'Status' => 'En Proceso', 'Incorrecto' => 0,
        ]);
        $filas = [];
        for ($i = 1; $i <= 10; $i++) {
            $filas[] = [
                ...
                'Folio' => '00700',
                'Hilos' => 400,
                // solo los renglones 1 y 2 estan capturados
                'HoraInicial' => $i <= 2 ? '06:00' : null,
                'NoJulio' => $i <= 2 ? "J{$i}" : null,
                'KgBruto' => $i <= 2 ? 300 : null,
            ];
        }
        DB::connection('sqlsrv')->table('UrdProduccionUrdido')->insert($filas);
    }

    private function controladorUrdido(): ModuloProduccionUrdidoController
    {
