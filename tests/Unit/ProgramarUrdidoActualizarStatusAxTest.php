...
class ProgramarUrdidoActualizarStatusAxTest extends TestCase
{
    protected function setUp(): void
    {
        ...
        Schema::connection('sqlsrv')->create('UrdProduccionUrdido', function (Blueprint $table): void {
            $table->increments('Id');
            $table->string('Folio')->nullable();
    ...
    public function test_no_permite_cancelado_programado_ni_en_proceso_si_el_folio_tiene_ax(): void
    {
        ...
        foreach (['Cancelado', 'Programado', 'En Proceso'] as $status) {
            ...
            $this->assertSame(ProgramaConfig::MENSAJE_AX_BLOQUEA_ESTATUS, $payload['error']);
        }
    ...
    public function test_permite_cambiar_estatus_si_ax_no_esta_en_1(): void
    {
        ...
        $response = app(ProgramarUrdidoController::class)->actualizarStatus(Request::create(
            '/urdido/programar-urdido/actualizar-status',
            'POST',
        ...
        $this->assertSame('Programado', DB::connection('sqlsrv')->table('UrdProgramaUrdido')->where('Id', 1)->value('Status'));
    }
}
