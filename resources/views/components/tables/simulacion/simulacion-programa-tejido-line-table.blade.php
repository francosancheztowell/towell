<div id="simpt-line-wrapper" class="mt-4 hidden">
    <div class="shadow rounded-md overflow-hidden">
        ...
        <div class="overflow-x-auto max-h-48" style="max-height: 250px; overflow-y: auto;">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-stone-700 text-white sticky top-0">
                    <tr>
                ...
                <tbody id="simpt-line-body" class=" divide-y divide-gray-100 bg-white">
                </tbody>
            </table>
        </div>
    </div>
...
<script>
async function loadSimulacionProgramaTejidoLines(params = {}) {
    const wrap = document.getElementById('simpt-line-wrapper');
    const body = document.getElementById('simpt-line-body');
    const meta = document.getElementById('simpt-line-meta');
    if (!wrap || !body) return;

    const qs = new URLSearchParams(params).toString();
    const url = '/simulacion/req-programa-tejido-line' + (qs ? ('?' + qs) : '');
    ...
    try {
        ...
        if (!r.ok) {
            body.innerHTML = `<tr><td colspan="14" class="px-3 py-6">
                <div class="max-w-xl mx-auto  text-stone-800 rounded-md p-4 text-sm text-center">
...
window.loadSimulacionProgramaTejidoLines = loadSimulacionProgramaTejidoLines;
// También mantener compatibilidad con el nombre anterior si se necesita
window.loadReqProgramaTejidoLines = loadSimulacionProgramaTejidoLines;
</script>
