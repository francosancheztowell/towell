...
<body>
    ...
    {{-- <div class="folios">
        <span class="folio-badge folio-t1">Turno {{ ((($coberturaT4PorTurno ?? [])['1']['cubierto'] ?? false) ? 4 : 1) }}: {{ $foliosPorTurno['1'] ?? '—' }}</span>
        <span class="folio-badge folio-t2">Turno {{ ((($coberturaT4PorTurno ?? [])['2']['cubierto'] ?? false) ? 4 : 2) }}: {{ $foliosPorTurno['2'] ?? '—' }}</span>
        <span class="folio-badge folio-t3">Turno {{ ((($coberturaT4PorTurno ?? [])['3']['cubierto'] ?? false) ? 4 : 3) }}: {{ $foliosPorTurno['3'] ?? '—' }}</span>
    </div> --}}
        ...
        <thead>
            ...
            <tr>
                @for ($t = 1; $t <= ($maxTurno ?? 3); $t++)
                    <th colspan="5" class="{{ $turnoHdr[$t - 1] }}">Turno {{ ((($coberturaT4PorTurno ?? [])[(string) $t]['cubierto'] ?? false) ? 4 : $t) }}</th>
                @endfor
            </tr>
