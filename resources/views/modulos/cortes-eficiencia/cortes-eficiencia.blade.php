                                ...
                                @foreach($horarios as $h => $c)
                                    <th colspan="3"
                                        ...
                                        <div class="flex items-center justify-center gap-2">
                                            <span>{{ $c['title'] }}</span>
                                            <button type="button"
                                                class="p-1 rounded focus:outline-none {{ $c['hover'] }} text-white"
                                                data-action="tomar-hora" data-horario="{{ $h }}"
                                                title="Tomar hora Horario {{ $h }}">
        ...
        (() => {
            ...
            function bindEvents() {
                ...
                document.querySelectorAll('.valor-input').forEach(input => {
                    ...
                    // Navegación con Enter: RPM -> EF (misma fila) -> RPM (siguiente fila), mismo horario
                    input.addEventListener('keydown', (e) => {
                        ...
                        const tipo = input.dataset.type;
                        const activos = (selector) => Array.from(document.querySelectorAll(selector))
                            .filter(el => !el.disabled && !el.readOnly);

                        const rpmInputs = activos(`input.valor-input[data-horario="${horario}"][data-type="rpm"]`);
                        const efInputs = activos(`input.valor-input[data-horario="${horario}"][data-type="eficiencia"]`);

                        let siguiente = null;
                        if (tipo === 'rpm') {
                            // Desde RPM: ir a EF de la misma fila (mismo telar y horario)
                            const telar = input.dataset.telar;
                            siguiente = document.querySelector(`input.valor-input[data-telar="${telar}"][data-horario="${horario}"][data-type="eficiencia"]`);
                            if (siguiente && (siguiente.disabled || siguiente.readOnly)) siguiente = null;
                        } else if (tipo === 'eficiencia') {
                            // Desde EF: ir al RPM de la siguiente fila en el mismo horario
                            const idx = efInputs.indexOf(input);
                            if (idx !== -1) siguiente = rpmInputs[idx + 1] || null;
                        }
