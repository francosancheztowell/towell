...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function cargarProducciones(telarId) {
        ...
        fetch(`/desarrolladores-muestras/telar/${telarId}/producciones`)
            .then(r => r.json())
            .then(data => {
    ...
    function cargarResumenCatCodificados(telarId, noProduccion) {
        ...
        fetch(`/desarrolladores-muestras/catcodificados/${encodeURIComponent(telarId)}/${encodeURIComponent(noProduccion)}`)
            .then(r => r.json())
    ...
    function buscarYActualizarCodigoDibujo(salon, telar, tamano) {
        ...
        if (salon && tamano) {
            fetch(`/desarrolladores-muestras/modelo-codificado/${encodeURIComponent(salon)}/${encodeURIComponent(tamano)}`)
                .then(r => r.json())
    ...
    function enviarFormulario() {
        ...
        fetch('/desarrolladores-muestras', { method: 'POST', body: formData, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
            .then(r => r.json())
            .then(data => {
                ...
                Swal.fire({ icon: 'success', title: '¡Guardado exitosamente!', text: data.message || 'Los datos se han guardado correctamente', confirmButtonColor: '#2563eb', confirmButtonText: 'Aceptar' })
                    .then(() => {
                        window.location.href = "{{ route('tejedores.desarrolladores-muestras') }}";
                    });
            })
            .catch(error => {
