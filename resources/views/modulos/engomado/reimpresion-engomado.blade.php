                ...
                <tbody class="divide-y divide-gray-200" id="tbodyOrdenes">
                    @forelse ($ordenes as $orden)
                        ...
                        >
                            ...
                            <td class="px-2 py-2" data-value="{{ $orden->MaquinaEng ?? '' }}">{{ $orden->MaquinaEng ?? '-' }}</td>
    ...
    <div class="bg-white max-w-xl w-full rounded-xl shadow-xl p-3 m-3">
        ...
        <div class="grid grid-cols-2 gap-3 mb-4">
            <div class="p-4 rounded-lg border-2 border-blue-300 bg-blue-50">
                <label class="block text-md font-semibold text-blue-800 mb-2 text-center">Folio</label>
            ...
            <div class="p-4 rounded-lg border-2 border-green-300 bg-green-50">
                <label class="block text-md font-semibold text-green-800 mb-2 text-center">Máquina</label>
                <select id="filter-maquina" class="w-full rounded border border-green-300 px-2 py-1.5 text-sm focus:ring-2 focus:ring-green-500 bg-white">
                    ...
                    @php
                        $maquinas = $ordenes->pluck('MaquinaEng')->filter()->unique()->sort()->values();
                    @endphp
            ...
            <div class="p-4 rounded-lg border-2 border-cyan-300 bg-cyan-50">
                <label class="block text-md font-semibold text-cyan-800 mb-2 text-center">Tipo</label>
            ...
            <div class="p-4 rounded-lg border-2 border-amber-300 bg-amber-50">
                <label class="block text-md font-semibold text-amber-800 mb-2 text-center">Status</label>
...
<script>
    (() => {
        ...
        window.seleccionarFila = function(row) {
            document.querySelectorAll('#tbodyOrdenes tr.table-row').forEach(r => {
        ...
        window.editarOrdenSeleccionada = function() {
            ...
            const baseUrl = '{{ route('engomado.editar.ordenes.programadas') }}';
            const url = baseUrl + '?orden_id=' + encodeURIComponent(String(ordenSeleccionada.id).trim()) + '&from=reimpresion';
        ...
        function ordenarTabla(columna, orden) {
            ...
            const filas = Array.from(tbody.querySelectorAll('tr.table-row'));
            const ordenIdSeleccionada = ordenSeleccionada ? ordenSeleccionada.id : null;
            const headers = document.querySelectorAll('#tablaOrdenes thead th[data-col]');
            let colIndex = -1;
            headers.forEach((h, idx) => {
                if (h.dataset.col === columna) colIndex = idx;
            });
            ...
            filas.sort((a, b) => {
                ...
                let comparacion = 0;
                if (esNumero) comparacion = aNum - bNum;
                else if (columna === 'fecha' && /^\d{4}-\d{2}-\d{2}$/.test(aValue) && /^\d{4}-\d{2}-\d{2}$/.test(bValue))
                    comparacion = aValue.localeCompare(bValue);
                else comparacion = aValue.localeCompare(bValue, 'es', { numeric: true, sensitivity: 'base' });
            ...
            headers.forEach(h => {
                ...
                if (h.dataset.col === columna) {
                    ...
                    if (icon) icon.className = orden === 'asc' ? 'fas fa-sort-up ml-1 text-xs' : 'fas fa-sort-down ml-1 text-xs';
                } else {
                    h.dataset.order = '';
                    if (icon) icon.className = 'fas fa-sort ml-1 text-xs opacity-50';
                }
            });
        ...
        function actualizarContador() {
            const filas = document.querySelectorAll('#tbodyOrdenes tr.table-row');
            const contador = document.getElementById('contadorRegistros');
            if (contador) {
                const count = Array.from(filas).filter(f => f.style.display !== 'none' && !f.querySelector('td[colspan]')).length;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (fechaHeader) ordenarTabla('fecha', 'desc');
            ...
            document.querySelectorAll('#tablaOrdenes thead th[data-col]').forEach(header => {
                header.addEventListener('click', function() {
                    ...
                    let nuevoOrden = 'asc';
                    if (ordenActual === 'asc') nuevoOrden = 'desc';
                    else if (ordenActual === 'desc') nuevoOrden = 'asc';
        ...
        function initFilters() {
            ...
            let filterState = { folio: '', maquina: '', tipo: '', status: '' };
            ...
            function applyFilters() {
                ...
                rows.forEach(row => {
                    ...
                    if (filterState.folio && !folio.includes(filterState.folio.toLowerCase())) show = false;
                    if (show && filterState.maquina && maquina !== filterState.maquina) show = false;
                    if (show && filterState.tipo && tipo !== filterState.tipo.toLowerCase()) show = false;
                    if (show && filterState.status && status !== filterState.status) show = false;
                ...
                if (visibleCount === 0) {
                    if (!noResultsRow) {
                        ...
                        tr.innerHTML = `<td colspan="7" class="px-2 py-4 text-center text-gray-500"><div class="flex flex-col items-center gap-2"><i class="fa-solid fa-inbox text-4xl text-gray-300"></i><span class="text-base font-medium">No hay órdenes con los filtros aplicados</span></div></td>`;
                        tbody?.appendChild(tr);
                    }
                } else noResultsRow?.remove();

                actualizarContador();
                const activeCount = Object.values(filterState).filter(v => v !== '').length;
                if (activeCount > 0 && filterBadge) { filterBadge.textContent = activeCount; filterBadge.classList.remove('hidden'); }
                else if (filterBadge) filterBadge.classList.add('hidden');
            }

            btnOpenFilters?.addEventListener('click', () => { modalFilters?.classList.remove('hidden'); modalFilters?.classList.add('flex'); });
            btnCloseFilters?.addEventListener('click', () => { modalFilters?.classList.add('hidden'); modalFilters?.classList.remove('flex'); });
            modalFilters?.addEventListener('click', (e) => { if (e.target === modalFilters) { modalFilters.classList.add('hidden'); modalFilters.classList.remove('flex'); } });

            filterFolio?.addEventListener('input', (e) => { filterState.folio = e.target.value.trim(); applyFilters(); });
            filterMaquina?.addEventListener('change', (e) => { filterState.maquina = e.target.value; applyFilters(); });
            filterTipo?.addEventListener('change', (e) => { filterState.tipo = e.target.value; applyFilters(); });
            filterStatus?.addEventListener('change', (e) => { filterState.status = e.target.value; applyFilters(); });
            btnClearFilters?.addEventListener('click', () => {
                filterFolio.value = ''; filterMaquina.value = ''; filterTipo.value = ''; filterStatus.value = '';
