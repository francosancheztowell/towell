...
<section class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm" aria-labelledby="titulo-refacciones-paro">
    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 px-3 py-2.5 short:py-1.5 sm:px-4">
        <div>
    ...
    @if ($estadoRefacciones !== 'ok')
        <p class="px-3 py-6 short:py-3 text-center text-sm {{ $estadoRefacciones === 'error' ? 'text-amber-800' : 'text-gray-500' }} sm:px-4">
        ...
        <div class="max-w-full overflow-x-auto overscroll-x-contain" tabindex="0" aria-label="Tabla de refacciones del paro">
            <table class="min-w-[720px] w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50 text-xs font-semibold text-gray-600">
                    <tr>
                        <th class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-left">Folio</th>
                        <th class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-center">Fecha</th>
                        <th class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-left">Status</th>
                        <th class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-left">Artículo</th>
                        <th class="min-w-52 px-3 py-2.5 short:py-1.5 text-left">Nombre</th>
                        <th class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-right">Cantidad</th>
                        <th class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-right">Importe</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 bg-white">
                    @foreach ($filasRefacciones as $fila)
                        <tr class="transition hover:bg-gray-50">
                            <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5">
                                <span class="inline-flex rounded-md bg-gray-900 px-2 py-0.5 text-xs font-bold text-white">{{ $fila['folio'] }}</span>
                            </td>
                            <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-center text-gray-700">{{ $fila['fecha'] }}</td>
                            <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5">
                            ...
                            <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5 font-medium text-gray-800">{{ $fila['articulo'] !== '' ? $fila['articulo'] : '—' }}</td>
                            <td class="px-3 py-2.5 short:py-1.5 text-gray-800">{{ $fila['nombre'] !== '' ? $fila['nombre'] : '—' }}</td>
                            <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-right tabular-nums text-gray-800">{{ $fila['cantidad'] }}</td>
                            <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-right tabular-nums text-gray-800">{{ $fila['importe'] }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="bg-gray-50 text-sm font-semibold text-gray-800">
                    <tr>
                        <td colspan="5" class="px-3 py-2.5 short:py-1.5 text-right text-xs uppercase tracking-wide text-gray-500">Total</td>
                        <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-right tabular-nums">{{ $totalCantidadVisible }}</td>
                        <td class="whitespace-nowrap px-3 py-2.5 short:py-1.5 text-right tabular-nums">{{ number_format((float) $refacciones['totalImporte'], 2, '.', ',') }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
