...
@section('navbar-right')
    ...
    @if (!empty($fechaIni) && !empty($fechaFin))
        <a href="{{ route('urdido.reportes.urdido.resumen.excel', ['fecha_ini' => $fechaIni, 'fecha_fin' => $fechaFin]) }}"
...
@section('content')
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 bg-white border border-t-0 border-gray-300 rounded-b-lg p-4">
            ...
            <div class="w-full space-y-6">
                @if (!empty($datosSemanales))
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <canvas id="eficienciaChartUrdido"></canvas>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div id="modalConsultarResumenUrdido" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
            <div class="bg-blue-600 text-white px-6 py-4 rounded-t-lg flex items-center justify-between">
                <h3 class="text-lg font-semibold">Consultar Resumen Urdido</h3>
                <button type="button" onclick="cerrarModalConsultarResumenUrdido()" class="text-white hover:text-gray-200">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>

            <form method="GET" action="{{ route('urdido.reportes.urdido.resumen') }}" class="p-6">
                <div class="space-y-4">
                    <div>
                ...
                <div class="flex gap-3 mt-6">
                    <button type="button" onclick="cerrarModalConsultarResumenUrdido()"
...
<script>
    ...
    function cerrarModalConsultarResumenUrdido() {
        document.getElementById('modalConsultarResumenUrdido').classList.add('hidden');
    }

    document.getElementById('modalConsultarResumenUrdido').addEventListener('click', function(e) {
        if (e.target === this) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const ctxEficiencia = document.getElementById('eficienciaChartUrdido').getContext('2d');
