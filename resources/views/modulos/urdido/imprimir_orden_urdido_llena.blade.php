        ...
        <div class="w-full overflow-x-auto">
            <table class="border border-collapse border-black mb-1 text-center w-full">
                ...
                <tbody class="leading-none">
                    @php
                        $total = max(10, count($ordUrdido)); // muestra todos los registros, mínimo 10
                    @endphp

                    @for ($i = 0; $i < $total; $i++)
                        @php
                            $fila = $ordUrdido[$i] ?? null;
                            $padding = is_null($fila) ? 'p-[10px]' : 'p-[1px]';
                        @endphp
                        <tr>
                            <td class="border border-black {{ $padding }}">
                                {{ $fila?->hora_inicio ? \Carbon\Carbon::parse($fila->hora_inicio)->format('d-m-y') : '' }}
                            </td>
                            <td class="border border-black {{ $padding }}">
                                <strong>{{ $fila->oficial ?? '' }}</strong>
                            </td>
                            <td class="border border-black {{ $padding }}">{{ $fila->turno ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">
                                {{ $fila?->hora_inicio ? \Carbon\Carbon::parse($fila->hora_inicio)->format('d-m-y') : '' }}
                            </td>
                            <td class="border border-black w-14 {{ $padding }}">
                                {{ $fila?->hora_fin ? \Carbon\Carbon::parse($fila->hora_fin)->format('d-m-y') : '' }}
                            </td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->no_julio ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->hilos ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->peso_bruto ?? '' }}
                            </td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->tara ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->peso_neto ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->metros ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->hilatura ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->maquina ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->operacion ?? '' }}</td>
                            <td class="border border-black w-14 {{ $padding }}">{{ $fila->transferencia ?? '' }}
                            </td>
                        </tr>
                    @endfor
