        ...
        (() => {
            ...
            const bindProduccionInput = (input) => {
                ...
                const enviarProduccion = function () {
                    ...
                    if (field === 'h_inicio' || field === 'h_fin') {
                        ...
                        fetch('{{ route('urdido.modulo.produccion.urdido.actualizar.fecha') }}', {
                            method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrfToken},
                            body: JSON.stringify({ registro_id: registroId, fecha: valor })
            ...
            const agregarRegistrosProduccionATabla = (registros = []) => {
                ...
                registros.forEach((registro) => {
                    ...
                    tr.innerHTML = `
                        <td class="px-1 py-1 text-center">
                            <input type="date" data-field="fecha" data-registro-id="${registroId}"
                                value="${fechaVal}"
                        ...
                        <td class="px-0.5 py-0.5 text-center">
                            <input type="time" data-field="h_inicio" data-registro-id="${registroId}" value="" class="produccion-input w-24 max-w-24 px-0.5 py-0 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-blue-500">
                        </td>
                        <td class="px-0.5 py-0.5 text-center">
                            <input type="time" data-field="h_fin" data-registro-id="${registroId}" value="" class="produccion-input w-24 max-w-24 px-0.5 py-0 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-blue-500">
                        </td>
            ...
            const eliminarRegistrosProduccionDeTabla = (ids = []) => {
                ...
                if (filasConRegistro === 0) {
                    ...
                    trVacio.innerHTML = '<td colspan="14" class="px-3 py-2 text-center text-gray-500">No hay registros de produccion</td>';
