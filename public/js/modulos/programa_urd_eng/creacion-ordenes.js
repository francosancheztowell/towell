...
/* =================== Config & Utils =================== */
const DEBUG = false;
const log = (...a) => { if (DEBUG) console.log('[CO]', ...a); };
...
const fmtDate = (dateStr) => {
    ...
    try {
        ...
        return Number.isNaN(d.getTime()) ? dateStr : d.toLocaleDateString('es-MX', { day:'2-digit', month:'2-digit', year:'numeric' });
...
window.crearOrdenes = async function() {
    try {
        ...
        if (!filaSeleccionadaId || !gruposData[filaSeleccionadaId]) {
            alert('Por favor, seleccione una fila de la tabla principal.');
            return;
        }
        ...
        if (isBlank(bomId)) {
            alert('Por favor, ingrese un BOM ID (L.Mat Urdido) para la fila seleccionada.');
            return;
        }
        ...
        if (!checkboxesMarcados.length) {
            alert('Por favor, seleccione al menos un material de engomado.');
            return;
        }
        ...
        if (!construccionUrdido.length) {
            alert('Por favor, complete al menos una fila de construcción urdido (No. Julios o Hilos).');
            return;
        }
        ...
        if (camposFaltantes.length > 0) {
            // Mostrar mensaje con valores actuales para depuración
            const mensaje = `Por favor, complete los siguientes campos requeridos de Datos de Engomado:\n\n${camposFaltantes.join('\n')}\n\nValores detectados:\n- Núcleo: "${datosEngomado.nucleo || '(vacío)'}"\n- No. de Telas: "${datosEngomado.noTelas || '(vacío)'}"\n- Ancho Balonas: "${datosEngomado.anchoBalonas || '(vacío)'}"\n- Metraje de Telas: "${datosEngomado.metrajeTelas || '(vacío)'}"\n- Cuendeados Mín: "${datosEngomado.cuendeadosMin || '(vacío)'}"\n- Máquina Engomado: "${datosEngomado.maquinaEngomado || '(vacío)'}"\n- L Mat Engomado: "${datosEngomado.lMatEngomado || '(vacío)'}"`;
            alert(mensaje);

            console.error('Validación fallida - Campos faltantes:', camposFaltantes);
            console.log('Valores de datosEngomado:', datosEngomado);
            console.log('Valores directos de inputs:', {
                nucleo: inputNucleo?.value,
                noTelas: inputNoTelas?.value,
                anchoBalonas: inputAnchoBalonas?.value,
                metrajeTelas: inputMetrajeTelas?.value,
                cuendeadosMin: inputCuendeadosMin?.value,
                maquinaEngomado: inputMaquinaEngomado?.value,
                lMatEngomado: inputLMatEngomado?.value
            });
            console.log('Estado de selects:', {
                nucleoSelectedIndex: inputNucleo?.selectedIndex,
                anchoBalonasSelectedIndex: inputAnchoBalonas?.selectedIndex,
                maquinaEngomadoSelectedIndex: inputMaquinaEngomado?.selectedIndex
            });
        ...
        if (result.success) {
            alert(`Órdenes creadas exitosamente.\nFolio: ${result.data.folio}\nFolio Consumo: ${result.data.folioConsumo}`);
            // Opcional: redirigir o recargar la página
            // window.location.reload();
        } else {
            alert(`Error al crear órdenes: ${result.error || 'Error desconocido'}`);
        }

    } catch (error) {
        console.error('Error al crear órdenes:', error);
        alert(`Error al crear órdenes: ${error.message}`);
